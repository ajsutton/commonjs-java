    ...
    function tokenizer($TEXT, filename, html5_comments, shebang) {
        ...
        var read_string = with_eof_error("Unterminated string constant", function(quote_char) {
            ...
            for (;;) {
                ...
                } else if ("\r\n\u2028\u2029".indexOf(ch) >= 0) parse_error("Unterminated string constant"); else if (ch == quote) break;
                ret += ch;
            }
                ...
                switch (tmp = S.token.value, next(), tmp) {
                  ...
                  case "do":
                    return new AST_Do({
                        ...
                        condition: (expect_token("keyword", "while"), tmp = parenthesised(), semicolon(true),
                        tmp)
                    });
            ...
            switch (tok.type) {
              ...
              case "operator":
                if (!is_identifier_string(tok.value)) {
                    throw new JS_Parse_Error("Invalid getter/setter name: " + tok.value, tok.file, tok.line, tok.col, tok.pos);
                }
                ret = _make_symbol(AST_SymbolRef);
                break;
            }
                ...
                for (var stat_index = statements.length; --stat_index >= 0; ) {
                    ...
                    for (var var_defs_index = var_defs.length; --var_defs_index >= 0; ) {
                        ...
                        var tt = new TreeTransformer(function preorder(node) {
                            ...
                            if (side_effects_encountered |= node.has_side_effects(compressor)) return unwind = true,
                            node;
